...
export class AdventureAddNewComponent implements OnInit {
  ...
  CheckForErrors(): boolean {
    ...
    if (nameErr || locationErr || promoErr || capacityErr || steetErr || cityErr || countryErr || bioErr || priceErr) {
      return true;
    }
    else {
      return false;
  ...
  locationHasError(): boolean {
    if (this.adventure.latitude == '' || this.adventure.longitude == '') {
      this.messageService.showMessage("Morate uneti lokaciju na mapi", MessageType.ERROR);
      return true;
    }      
